...
def baseline_signals() -> list[CapabilitySignal]:
    ...
    return [
        ...
        CapabilitySignal(CognitiveDomain.REASONING, "world-aware bounded consequence simulation", 0.61, "structured plans are independently critiqued and deterministically simulated against bounded immutable current-world snapshots for prerequisites, expected state changes, failure modes and rollback properties; multi-step causal counterfactual reasoning remains limited"),
        ...
        CapabilitySignal(CognitiveDomain.WORLD_MODEL, "evidence-aware grounded state graph with bounded snapshots", 0.43, "durable temporal facts support confidence/provenance-aware grounding plus immutable bounded current-state snapshots for cognition-only consumers; richer relationships and predictive state transitions remain limited"),
